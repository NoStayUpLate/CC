        ...
        <div className="max-h-[calc(88vh-72px)] overflow-y-auto px-6 py-5 text-sm leading-relaxed text-black">
          <Section title="一、业务价值">
            ...
            <Bullets
              items={[
                ...
                <>决策依据：经验直觉 → <strong>GHI 三维加权 + 标签四象限定位</strong></>,
              ]}
            />
          </Section>

          <Section title="二、创新度">
            <P>不是又一个「爬数据看榜单」的工具，本系统在三个层面有原创设计：</P>

            <SubSection title="1. GHI 算法（自研）">
            ...
            <SubSection title="2. 选品罗盘（标签热度散点四象限）">
              <P>
            ...
            <SubSection title="3. 黄金三秒钩子（has_hook）">
              <P>
          ...
          <Section title="关键路径速查（开发者视角）">
            <Table
              head={["关注点", "入口文件"]}
              rows={[
                ["GHI 算法 SQL", "backend/routers/novels.py"],
