...
ECS 默认只开 22 端口（SSH），看板的 HTTP 端口必须手动开放。

### 2.1 选 HTTP 端口（关键决策）

| 你的情况 | 选什么端口 | 备注 |
|---------|-----------|------|
| 普通 ECS，能自由开放 80/443 | **80**（默认） | 标准 HTTP，浏览器不用带端口后缀 |
| 阿里云**轻量服务器**，只允许某个段（如 3000-4000） | **段内任选**，常用 3000 | 浏览器访问要加端口：`http://<IP>:3000` |
| 公司有合规要求只能用某个固定端口 | 那个固定端口 | 同上 |

> 怎么查自家服务器的开放范围？阿里云控制台 → 实例 → 安全组 → 入方向规则。

### 2.2 操作步骤（标准 ECS）

1. 浏览器登录 <https://ecs.console.aliyun.com>（轻量服务器入口稍有不同）
2. 左侧 **实例与镜像 → 实例**
3. 找到你的实例 → 点实例名进去
4. 切到 **安全组** 标签页 → 点关联的安全组名字
5. **入方向 → 手动添加**：

| 端口范围 | 授权对象 | 描述 |
|---------|---------|------|
| `80/80` | `0.0.0.0/0` | HTTP（默认部署模式） |
| `443/443` | `0.0.0.0/0` | HTTPS（将来加 Caddy 时用） |

或如果走非标端口：

| 端口范围 | 授权对象 | 描述 |
|---------|---------|------|
| `3000/3000`（举例） | `0.0.0.0/0` | 看板 HTTP（避开 80） |

6. 保存。**安全组规则秒级生效**，不用重启 ECS。

### 2.3 怎么把端口配到 docker compose 里

第 5 章会教在 `.env.production` 加一行：

```env
HOST_HTTP_PORT=3000
```

不填默认 80。改完跑 `./deploy.sh up` 自动生效。
...
| `HOST_HTTP_PORT` | 默认 `80`；安全组只开 3000-4000 段时改成 `3000` | **必须与第 2 章选的端口一致**，否则浏览器访问不到 |
...
| 你的 `HOST_HTTP_PORT` | 浏览器输入 |
|---------------------|-----------|
| `80`（默认） | `http://<ECS公网IP>` |
| `3000`（或其他非标端口） | `http://<ECS公网IP>:3000` |

`./deploy.sh status` 输出的"访问地址"那一行已经把端口拼好了，直接复制粘贴最稳。
...
**原因**：阿里云安全组没开访问端口 / 用了非标端口忘了带后缀
**解决**：
- 确认 `.env.production` 的 `HOST_HTTP_PORT` 与安全组开放的端口**完全一致**
- 浏览器要带端口后缀（`http://<IP>:3000`），不要省
- 改完 `.env` 后必须 `docker compose --env-file .env.production up -d --force-recreate frontend`，否则容器还在老端口监听
