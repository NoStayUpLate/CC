...
| **S_recency** | 数据新鲜度 | `100 - 10 × date_diff('day', crawl_date, current_date)`，10 天前的榜单归零，避免老数据虚高 |
...
| **数据建模** | DuckDB 列式嵌入式数据库 + 主键 + ON CONFLICT DO UPDATE 实现 rescrape upsert，**GHI / DHI 两层嵌套 SQL 在数据库内算**（前端零计算）；数据缺失契约严格：**统一 None 禁止 0 占位**，否则会污染评分排名 |
...
| **部署** | Docker Compose 瘦身后只剩 2 个容器（nginx + FastAPI），单文件 DuckDB 嵌进 backend，整体 1C2G 服务器即可流畅跑（~250MB 内存）；国内 ECS 友好的镜像源优化（apt → 阿里云 / pip → 阿里云 / npm → npmmirror）|
| **架构演进史** | 早期版本是 ClickHouse 同栈，1C2G 服务器跑不动 + 阿里云 NUMA seccomp 拦截，最终切换到 DuckDB 嵌入式（commit `5dcbb23` 完成切换 + 数据迁移脚本 `scripts/migrate_ch_to_duckdb.py`） |
| **AI 协作工程化** | 项目根有 [CLAUDE.md](../CLAUDE.md)（AI 工作手册）+ `.claude/skills/` 自定义 skill（爬虫开发流程 / 项目加固审计），让 AI 能按规范长期协作而不破坏架构契约 |

**代码规模参考**：后端约 5000 行 Python（含 11 个爬虫子类）、前端约 3500 行 React、DuckDB DDL 含主键约束 + 批量写入 + 锁封装约 200 行。
...
| 表结构 + DuckDB 连接管理 | [backend/database.py](../backend/database.py) |
| ClickHouse → DuckDB 一次性迁移脚本 | [scripts/migrate_ch_to_duckdb.py](../scripts/migrate_ch_to_duckdb.py) |
...
| 生产部署快照（4→2 容器精简） | [docs/PRODUCTION.md](PRODUCTION.md) |
